...
PLAYFAB_SECRET_KEY=$(imp-ci secrets read --environment=production --buildkite-org=improbable --secret-type=generic-token --secret-name="online-services-playfab-secret-key-integ" | jq -Mr '.token')
sh test.sh --test_all
popd
