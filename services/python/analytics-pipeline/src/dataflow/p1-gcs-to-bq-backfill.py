...
from common.functions import datesGenerator, gcsFileListGenerator, listToSqlTuple
...
def run():
    ...
    fileListBq = (p1 | 'parseBqFileList' >> beam.io.Read(beam.io.BigQuerySource(
                        ...
                        query=queryGenerator(args.gcp, method, args.event_ds_start, args.event_ds_stop, listToSqlTuple(list_time_part), listToSqlTuple(list_env), args.event_category, args.scale_test_name),
    ...
    logsList = (parseList | 'addParseInitiatedInfo' >> beam.Map(lambda x: {'job_name': job_name,
                                                                           ...
                                                                           'analytics_environment': pathParser(x, 'analytics_environment='),
                                                                           'event_category': pathParser(x, 'event_category='),
                                                                           'event_ds': pathParser(x, 'event_ds='),
                                                                           'event_time': pathParser(x, 'event_time='),
